...
class Reset_password extends CI_Controller {
    ...
    public function index() {
        ...
        if(filter_input_array(INPUT_POST)){
            ...
            if(! $this->form_validation->run() == FALSE){
                ...
                $column = array('username', 'email', 'is_approved_by_admin');
                $arr = array('email' => $email_address);
                
                $record = $this->user->getSpecificColumnRec($column, $arr);
                ...
                if(!empty($record)){
                    ...
                    foreach ($record as $index => $column) {
                        ...
                        $already_verified = $column['is_approved_by_admin'];
                    }
    ...
    public function update(){
        ...
        if(filter_input_array(INPUT_POST)){
            ...
            if(!$this->form_validation->run($rules) == FALSE){
                ...
                $arr = array('username' => $user_name);
                $record = $this->user->getSpecificColumnRec('email', $arr);
                $email_address = '';
    ...
    private function send_resetConfirm_email($user_name, $userEmail){
        ...
        $this->load->library('email');
        $this->load->model('send_email');
        $success = $this->send_email->send($userEmail, $message);
        return $success;
